...
import { Event, eventYouTubeVideosTag, listEventYouTubeVideos } from '@/db/events'
...
import EventYouTubeVideoEditor from './EventYouTubeVideoEditor'

interface Props {
  event: Event
}

const EventYouTubeVideoEditorWrapper = async ({ event }: Props) => {
  const videos = await listEventYouTubeVideos(event.id)()
  return <EventYouTubeVideoEditor event={event} videos={videos} />
}

export default EventYouTubeVideoEditorWrapper
...
export const eventYouTubeVideoEditorAction = async (
  state: State,
  data: FormData
): Promise<State> => {
  ...
  if (addId && addId !== '') {
    ...
    try {
      await executeQueryWithLogging(
        prisma.event_youtube_videos.create(params),
        'event_youtube_videos.create',
        params
      )
      revalidateTag(eventYouTubeVideosTag(event_id))
  ...
  if (delId && delId !== '') {
    ...
    try {
      await executeQueryWithLogging(
        prisma.event_youtube_videos.deleteMany(params),
        'event_youtube_videos.delete',
        params
      )
      revalidateTag(eventYouTubeVideosTag(event_id))
      return { ...state, error: undefined }
